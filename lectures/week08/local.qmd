...
jupyter: python3
execute:
  cache: true  # Enables caching for the entire document
---
...
san_diego.plot('spatial_clusters', cmap='tab20', categorical=True, legend=True)
```

```{python}
san_diego['spatial_clusters'] = gint.component_labels
san_diego.explore('spatial_clusters', cmap='tab20', categorical=True, legend=True, tooltip=['spatial_clusters', 'cluster_type'])
```
```{python}
m = san_diego.loc[[626]].explore(color="#25b497")
san_diego.loc[[626]].explore(m=m, color="#fa94a5")
gint.explore(san_diego,m=m, focal='626')
```
